# Generic Agent Consolidation Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace CodeRig's planner, builder, and reviewer topology with one lean `generic` primer that can delegate to itself and optionally launch Codex or Claude Code ACP children.

**Architecture:** CodeRig will construct one `loop.Definition` directly from one catalog package, one full coding toolset, and one session access set/gate. Runtime configuration will have no `delegate_defaults`; the in-process `looprig/native` row is the deterministic default and ACP rows are explicit alternatives for `generic`.
...
### Task 1: Add the Generic identity and prompt

**Files:**
- Create: `internal/catalog/generic/generic_test.go`
- Create: `internal/catalog/generic/generic.go`
...
**Step 1: Write the failing Generic catalog test**

Create a test that requires `Name == "generic"`, a non-empty description, one well-formed `<identity product="CodeRig">` prompt, and the approved intent/workflow/tools/safety/delegation/communication sections. Assert the prompt contains `Generic`, `answer`, `change`, `verify`, `untrusted data`, `destructive`, and `delegate` once in the appropriate sections, rather than checking provider brand names.

```go
func TestIdentity(t *testing.T) {
    if Name != "generic" { t.Fatalf("Name = %q, want generic", Name) }
    var root struct {
    ...
    if root.Product != "CodeRig" { t.Fatalf("product = %q", root.Product) }
}
```

**Step 2: Run the test to verify it fails**

Run: `GOWORK=off go test ./internal/catalog/generic`
...
const SystemPrompt = `<identity product="CodeRig">...approved prompt...</identity>`
```

Remove the old shared `catalog.Identity` constant once assembly consumes `generic.SystemPrompt`; retain no provider-branded prompt fragments.

**Step 4: Run catalog tests**

Run: `GOWORK=off go test ./internal/catalog/...`

Expected: PASS for the new Generic catalog; old packages remain until Task 5 to keep intermediate builds coherent.
...
### Task 3: Compile one Generic runtime catalog with an in-process default
...
Require the production catalog to contain entries only for `generic`, exactly one default entry, and that default to be `looprig/native`. Verify both ACP harnesses remain non-default explicit choices when configured.
...
Add a Harness-level preparation integration assertion through CodeRig's assembled `StartAgent`: omitted runtime selectors resolve to `looprig/native`; explicit `agent_harness: "codex"` and `"claude-code"` resolve to their ACP profiles.
...
Make the compiler fixed to `generic.Name`; remove `AgentTypes` and `Defaults` inputs and `normalizedAgentTypes`. Produce raw ACP rows without assigning them product defaults, combine them with the ordinary `looprig/native` row, mark only that row `Default: true`, then call `loop.NewRuntimeCatalog` once. Do not retain an ACP-only catalog whose temporary default is later repaired.

The ordinary row uses configured delegate-capable model options and existing routing clients. If no delegate-capable model exists, use the configured primer model as the single in-process option so plain Generic delegation always exists. ACP rows remain conditional on their configured model/native profiles and executable preflight.
...
git commit -m "refactor: compile generic runtime catalog"
```

### Task 4: Collapse tools and access to one Generic capability set
...
Replace executor-separation assertions with a test that `sessionAccess` exposes one non-nil `set`, one `gate`, and one `policyRev`; repeated loop IDs memoize within that set; different Generic loop IDs receive different executors; and `Close` closes the set exactly once.

Add a tool roster test requiring Generic to expose the builder's complete tool set, including read/search, write/edit, supervised Bash/process tools, web/fetch, task/user interaction, and optional Skill.

**Step 2: Run focused tests to verify failure**

Run: `GOWORK=off go test ./internal/app -run 'SessionAccess|GenericTool|AcceptanceProfileGateBehavior'`
...
Build one executor set from the selected CodeRig profile and one `roleGate` from the same profile. Rename `rolePolicyRevision` to `agentPolicyRevision` (or inline it if used once) and identify it as `generic`. Compute the access digest from one effective profile; remove planner/reviewer restrictions and aliases.

Rename `builderToolDefinitions` to `genericToolDefinitions`, delete planner/reviewer/operator variants, and retain the full implementation roster with no branching by role.
...
### Task 5: Replace swarm assembly with one direct Generic definition
...
Require `genericDefinition` to return one definition named/displayed `generic`, with `generic` as its only delegate, managed delegation, quick/deep modes, Generic prompt, full tools, runtime context, access gate, and policy revision. Require session assembly to expose `generic` as the sole primer.

```go
definition, err := genericDefinition(client, model, cfg, access, nil)
if err != nil { t.Fatal(err) }
if definition.Name() != generic.Name { t.Fatalf("name = %q", definition.Name()) }
...
Use `loop.WithDelegates(generic.Name)` and the existing managed-delegation limits. Rename `swarmStores` and related provider identifiers to `sessionStores` where they describe storage rather than agent topology.
...
Run: `GOWORK=off go test ./internal/app -run 'Generic|Agent|Delegat|Skill|Acceptance|Persist'`
...
### Task 6: Restrict ACP and MCP role surfaces to Generic
...
- empty MCP `roles` exposes a binding to Generic;
- `roles: ["generic"]` is accepted;
- every former role name is rejected;
- Generic can start an in-process child without runtime selectors and explicit Codex/Claude children with selectors.
...
**Step 3: Implement the Generic-only surfaces**

Change ACP posture selection to one accepted name and one result. Set MCP's allowed/default role list to `generic` and remove role fan-out. Update event assertions and child fixtures to attribute all loops to Generic while still distinguishing runtime harness/profile.
...
Replace the old roster contract with source-level assertions that production contains `internal/catalog/generic`, has no imports of the deleted role packages, exposes `agentKind == "coderig:generic"`, and does not contain live `delegate_defaults`, `leafBuiltin`, or `swarmDefinitions` plumbing. Historical design documents may retain old terms.
...
Update fingerprints and persistence assertions to Generic. Remove compatibility aliases and obsolete helpers. Rewrite contributor/architecture guidance around one agent, one access authority, Generic self-delegation, in-process default selection, and explicit ACP alternatives. Do not rewrite historical plan documents.

Run `rg -n 'planner|builder|reviewer|delegate_defaults|leafBuiltin|swarmDefinitions' --glob '!docs/plans/**' --glob '!*.sum'` and classify every remaining hit: delete live product-role references; keep ordinary English uses such as a third-party builder type only when they are not CodeRig agent identities.

**Step 4: Run package tests**

Run: `GOWORK=off go test ./internal/app ./internal/catalog/... ./cmd/coderig`
...
Run: `gofmt -w internal/app internal/catalog cmd/coderig`
...
Run: `GOWORK=off GOCACHE=/private/tmp/coderig-generic-gocache go test -race ./internal/catalog/... ./internal/app -run 'Generic|Agent|Delegat|RuntimeCatalog|ModelConfig|Access|MCP|ACP|Fingerprint'`

Expected: PASS.

**Step 3: Run the full race-enabled suite**

Run: `GOWORK=off GOCACHE=/private/tmp/coderig-generic-gocache go test -race ./...`
...
Run: `GOWORK=off GOCACHE=/private/tmp/coderig-generic-gocache make lint`

Run: `GOWORK=off GOCACHE=/private/tmp/coderig-generic-gocache make test-integration`
