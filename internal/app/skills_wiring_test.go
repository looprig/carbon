...
func TestGenericSystemPromptOmitsEmbeddedSkillCatalog(t *testing.T) {
	t.Parallel()
	...
	if strings.Contains(system, "<available_skills>") || strings.Contains(system, "code-style") {
		t.Fatalf("Generic system prompt contains removed embedded skill catalog: %q", system)
	}
}

func TestGenericSkillDefinitionAlwaysUsesWorkspace(t *testing.T) {
	t.Parallel()
	def := skillDefinitionFor(testSkillLoader())
	if def == nil {
		t.Fatal("skillDefinitionFor() = nil, want Generic Skill definition")
	}
